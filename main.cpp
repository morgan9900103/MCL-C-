...
int main()
{
    ...
    for (int t = 0; t < steps; t++) {
        ...
        Robot particles_motion[n];
        for (int i = 0; i < n; i++) {
            particles_motion[i] = particles[i].move(0.1, 5.0);
            particles[i] = particles_motion[i];
        }

        // Generate particle weights depending on robot's measurement
        double weights[n];
        for (int i = 0; i < n; i++) {
            weights[i] = particles[i].measurement_prob(z);
            // cout << weights[i] << endl;
        }

        // Resample the particles with a sample probability proportional to the importance weight
        Robot particles_resample[n];
        ...
        for (int i = 0; i < n; i++) {
            ...
            particles_resample[i] = particles[index];
        }
        for (int i = 0; i < n; i++) {
            particles[i] = particles_resample[i];
            // cout << particles[i].show_pose() << endl;
        }

        // Evaluate the Error
        cout << "Step = " << t << ", Evaluation = " << evaluation(robot, particles, n) << endl;

        // Graph the position of the robot and the particles at each step
        visualization(n, robot, t, particles_motion, particles_resample);
